...
@register_tool
class TuneTokenClassificationCaller(TuneCaller):
    def __init__(self, **kwargs):
        super().__init__("saprot_tune_token_classification", **kwargs)

    def __call__(self, dataset, num_classes, base_model="SaProt_35M", batch_size="Adaptive", max_epoch=10, learning_rate=1e-3) -> dict:
        dataset = f"{self.out_dir}/{dataset}"
        
        # Create save directory following the same pattern as other callers
        start_time = datetime.datetime.now()
        timestamp = start_time.strftime("%Y%m%d_%H%M%S")
        work_dir = f"{self.out_dir}/saprot_tune_token_classification/{timestamp}"
        ...
        if not training_successful:
            return {"error": "Saprot token classification tuning failed. Model files not found."}
        else:
            duration = (datetime.datetime.now() - start_time).total_seconds()
            return {"save_dir": f"saprot_tune_token_classification/{timestamp}/{save_dir}",
...
if __name__ == "__main__":
    caller = TuneTokenClassificationCaller(out_dir=BASE_DIR)
    
    input_args = {
        "dataset": "example/[EXAMPLE][AminoAcidClassification-3Categories]Multiple_AA_Sequences.csv",
