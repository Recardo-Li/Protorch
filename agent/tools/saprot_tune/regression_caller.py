...
class TunePairRegressionCaller(TuneCaller):
    def __init__(self, **kwargs):
        super().__init__("saprot_tune_regression", **kwargs)
    ...
    def __call__(self, dataset, base_model="SaProt_35M", batch_size="Adaptive", max_epoch=10, learning_rate=1e-3) -> dict:
        ...
        work_dir = f"{self.out_dir}/saprot_tune_regression/{timestamp}"
        ...
        cmd_args = {
            ...
            "huggingface_path": f'{self.config["saprot_root"]}/{base_model}_AF2',
        ...
        if not training_successful:
            return {"error": "Saprot regression tuning failed. Model files not found."}
        else:
            duration = (datetime.datetime.now() - start_time).total_seconds()
            return {"save_dir": f"saprot_tune_regression/{timestamp}/{save_dir}",
...
if __name__ == "__main__":
    ...
    input_args = {
        "dataset": "example/[EXAMPLE][Regression]Multiple_AA_Sequences.csv",
