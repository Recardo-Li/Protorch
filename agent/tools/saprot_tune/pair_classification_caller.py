...
@register_tool
class TunePairClassificationCaller(TuneCaller):
    def __init__(self, **kwargs):
        super().__init__("saprot_tune_pair_classification", **kwargs)
    ...
    def __call__(self, dataset, num_classes, base_model="SaProt_35M", batch_size="Adaptive", max_epoch=10, learning_rate=1e-3) -> dict:
        ...
        work_dir = f"{self.out_dir}/saprot_tune_pair_classification/{timestamp}"
        ...
        if not training_successful:
            return {"error": "Saprot pair classification tuning failed. Model files not found."}
        else:
            duration = (datetime.datetime.now() - start_time).total_seconds()
            return {"save_dir": f"saprot_tune_pair_classification/{timestamp}/{save_dir}",
...
if __name__ == "__main__":
    caller = TunePairClassificationCaller(out_dir=BASE_DIR)
    
    input_args = {
        "dataset": "example/[EXAMPLE][Pair AA][Classification].csv",
