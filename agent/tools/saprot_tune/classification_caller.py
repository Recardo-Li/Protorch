...
@register_tool
class TuneClassificationCaller(TuneCaller):
    def __init__(self, **kwargs):
        super().__init__("saprot_tune_classification", **kwargs)
    ...
    def __call__(self, dataset, num_classes, base_model="SaProt_35M", batch_size="Adaptive", max_epoch=10, learning_rate=1e-3) -> dict:
        ...
        work_dir = f"{self.out_dir}/saprot_tune_classification/{timestamp}"
        ...
        if not training_successful:
            return {"error": "Saprot classification tuning failed. Model files not found."}
        else:
            duration = (datetime.datetime.now() - start_time).total_seconds()
            print(f"Model saved to: {work_dir}/{save_dir}")
            print(f"Training completed in {duration:.2f} seconds")
            return {"save_dir": f"saprot_tune_classification/{timestamp}/{save_dir}",
...
if __name__ == "__main__":
    caller = TuneClassificationCaller(out_dir=BASE_DIR)
    
    input_args = {
        "dataset": "example/[EXAMPLE][Classification-2Categories]Multiple_AA_Sequences.csv",
