...
@register_tool
class ProTrekStructure2Protein(BaseTool):
    def __init__(self, out_dir: str = f"{ROOT_DIR}/outputs/protrek_structure2protein", **kwargs):
        super().__init__(
            config_path=f"{BASE_DIR}/config.yaml",
            out_dir=out_dir,
            **kwargs
        )
        tool_name = "protrek_structure2protein"
    ...
    def __call__(self, foldseek_sequence, database="Swiss-Prot"):
        ...
        cmd_args = {
            ...
            "query_type": "sequence",
            "csv_path": f"{self.out_dir}/protrek/{now}-structure2protein.csv",
        }
        ...
        try:
            ...
            return {"error": f"ProTrek structure to protein search failed: {e}"}

if __name__ == '__main__':
    # Test
    tool = ProTrekStructure2Protein()
