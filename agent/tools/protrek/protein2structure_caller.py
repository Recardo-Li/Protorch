...
@register_tool
class ProTrekProtein2Structure(BaseTool):
    def __init__(self, out_dir: str = f"{ROOT_DIR}/outputs/protrek_protein2structure", **kwargs):
        super().__init__(
            config_path=f"{BASE_DIR}/config.yaml",
            out_dir=out_dir,
            **kwargs
        )
        tool_name = "protrek_protein2structure"
    ...
    def __call__(self, protein_sequence, database="Swiss-Prot") -> dict:
        ...
        cmd_args = {
            ...
            "csv_path": f"{self.out_dir}/protrek/{now}-protein2structure.csv",
        }
        ...
        try:
            ...
            return {"error": f"ProTrek protein to structure search failed: {e}"}

if __name__ == '__main__':
    # Test
    tool = ProTrekProtein2Structure()
