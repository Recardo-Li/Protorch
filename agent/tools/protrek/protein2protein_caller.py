...
@register_tool
class ProTrekProtein2Protein(BaseTool):
    def __init__(self, out_dir: str = f"{ROOT_DIR}/outputs/protrek_protein2protein", **kwargs):
        super().__init__(
            config_path=f"{BASE_DIR}/config.yaml",
            out_dir=out_dir,
            **kwargs
        )
        tool_name = "protrek_protein2protein"
    ...
    def __call__(self, protein_sequence, database="Swiss-Prot") -> dict:
        ...
        cmd_args = {
            ...
            "input_type": "sequence",
            "query_type": "sequence",
            "csv_path": f"{self.out_dir}/protrek/{now}-protein2protein.csv",
        }
        ...
        try:
            ...
            return {"protein_id": df.iloc[0]["Id"],
                    ...
                    "sequence_identity": df.iloc[0]["Sequence identity"],
                    "matching_score": float(df.iloc[0]["Matching score"]),
                    "duration": spend_time
                    }
        except Exception as e:
            return {"error": f"ProTrek protein2protein failed: {e}"}

if __name__ == '__main__':
    # Test
    tool = ProTrekProtein2Protein()
    
    input_args = {
        "protein_sequence": "MRLGSVFLVLLVLLGLGAGVAAVPGLG",
        "database": "Swiss-Prot",
    }
