...
@register_tool
class ProTrekProtein2Text(BaseTool):
    def __init__(self, out_dir: str = f"{ROOT_DIR}/outputs/protrek_protein2text", **kwargs):
        super().__init__(
            config_path=f"{BASE_DIR}/config.yaml",
            out_dir=out_dir,
            **kwargs
        )
        tool_name = "protrek_protein2text"
    ...
    def __call__(self, protein_sequence, database="Swiss-Prot", subsection="Function") -> dict:
        ...
        cmd_args = {
            ...
            "input_type": "sequence",
            "query_type": "text",
            "csv_path": f"{self.out_dir}/protrek/{now}-protein2text.csv",
        ...
        try:
            ...
            return {"error": f"ProTrek protein2text failed: {e}"}

if __name__ == '__main__':
    # Test
    tool = ProTrekProtein2Text()
