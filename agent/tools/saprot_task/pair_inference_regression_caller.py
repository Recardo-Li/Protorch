...
@register_tool
class SaProtPairInferenceRegressionCaller(SaProtCaller):
    def __init__(self, **kwargs):
        super(SaProtCaller, self).__init__(
            config_path=f"{BASE_DIR}/config.yaml",
            out_dir=f"{ROOT_DIR}/outputs/saprot_tuned_inference_pair_regression",
            **kwargs
        )
        for doc in self.config["document"]:
            if doc["tool_name"] == "saprot_tuned_inference_pair_regression":
                self.config["document"] = doc
                break
        self.tool_name = "saprot_tuned_inference_pair_regression"

    def __call__(self, protein1_sequence, protein2_sequence, model_dir, structure1_path=None, structure2_path=None) -> dict:
...
if __name__ == "__main__":
    caller = SaProtPairInferenceCaller()
