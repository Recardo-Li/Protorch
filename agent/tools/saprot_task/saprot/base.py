...
from data.utils.pdb2feature import batch_coords2feature
...
class SaprotBaseModel(AbstractModel):
    ...
    def plot_valid_metrics_curve(self, log_dict):
        """
        Plots and saves a graph of validation metrics.

        Parameters:
        - log_dict (dict): A dictionary containing metric names as keys and their values.
        ...
        for idx, metric in enumerate(log_dict.keys()):
            if type(log_dict[metric]) == torch.Tensor:
                ...
                value = log_dict[metric]
            
            # 将度量值添加到 valid_metrics_list 中
        ...
        # 保存图像到本地
        plt.savefig(f'{self.save_path}/validation_metric_curve.jpg')
