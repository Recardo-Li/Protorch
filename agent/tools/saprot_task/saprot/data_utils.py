...
def select_best_gpu():
    """
    Automatically select the GPU with the least memory usage
    Returns the device ID of the best GPU
    """
    ...
    try:
        ...
        if device_count == 1:
            print("[INFO] Only 1 GPU available, using GPU 0")
            return 0
            
        gpu_info = []
        for gpu_id in range(device_count):
            # Use torch.cuda.mem_get_info to get actual GPU memory usage
            # This includes memory used by all processes, not just current PyTorch process
            memory_free, memory_total = torch.cuda.mem_get_info(gpu_id)
            memory_used = memory_total - memory_free
            
            gpu_info.append((gpu_id, memory_used, memory_total, memory_free))
        
        if not gpu_info:
            print("[WARNING] Unable to get GPU information, using default GPU 0")
            return 0
        
        # Select GPU with most free memory
        best_gpu = max(gpu_info, key=lambda x: x[3])  # x[3] is memory_free
        gpu_id, memory_used, memory_total, memory_free = best_gpu
        ...
        print(f"[INFO] Auto-selected GPU {gpu_id}: Used memory {memory_used_mb}MB / {memory_total_mb}MB, Free {memory_free_mb}MB")
        return gpu_id
        
    except Exception as e:
        print(f"[WARNING] Error during GPU selection: {e}, using default GPU 0")
        return 0
...
model_type_dict = {
    "classification": "saprot_classification_model",
    "token_classification": "saprot_token_classification_model",
    "regression": "saprot_regression_model",
    "pair_classification": "saprot_pair_classification_model",
    "pair_regression": "saprot_pair_regression_model",
}
dataset_type_dict = {
...
def make_config(
    batch_size, max_epochs, learning_rate, base_model, task_type, csv_dataset_path, default_config_path, save_path
):
    # training config
    GPU_batch_size = 0
    ...
    if torch.cuda.is_available():
        ...
        if isinstance(best_gpu_id, int):
            config.Trainer.devices = [best_gpu_id]
            print(f"[INFO] Training will use GPU {best_gpu_id}")
        else:
            config.Trainer.devices = 1
            print(f"[INFO] Training will use default GPU settings")
    else:
        config.Trainer.devices = 1
