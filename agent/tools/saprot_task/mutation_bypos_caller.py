...
@register_tool
class SaProtMutationPosCaller(SaProtCaller):
    def __init__(self, **kwargs):
        super().__init__("saprot_mutation_bypos", **kwargs)

    def __call__(self, protein_sequence, mut_position, structure_path=None) -> dict:
        chain = "A"
        ...
        try:
            ...
            origin_aa = protein_sequence[pos]
            mutation_dict = eval(prediction)
            best_mutation = max(mutation_dict, key=mutation_dict.get)
            
            return {"pred": prediction, "origin_aa": origin_aa, "best_mutation": best_mutation}
        except Exception as e:
            return {"error": str(e)}
    
if __name__ == "__main__":
    caller = SaProtMutationPosCaller()
