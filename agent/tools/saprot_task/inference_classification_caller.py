...
@register_tool
class SaProtInferenceClassificationCaller(SaProtCaller):
    def __init__(self, **kwargs):
        ...
        for doc in self.config["document"]:
            if doc["tool_name"] == "saprot_tuned_inference_classification":
                self.config["document"] = doc
                break
        self.tool_name = "saprot_tuned_inference_classification"

    def __call__(self, protein_sequence, model_dir, num_labels, structure_path=None) -> dict:
...
if __name__ == "__main__":
    caller = SaProtInferenceClassificationCaller()
