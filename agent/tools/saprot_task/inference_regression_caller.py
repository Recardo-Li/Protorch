...
@register_tool
class SaProtInferenceRegressionCaller(SaProtCaller):
    def __init__(self, **kwargs):
        ...
        for doc in self.config["document"]:
            if doc["tool_name"] == "saprot_tuned_inference_regression":
                self.config["document"] = doc
                break
        self.tool_name = "saprot_tuned_inference_regression"

    def __call__(self, protein_sequence, model_dir, structure_path=None) -> dict:
        model_dir = f"{self.out_dir}/{model_dir}"
        print("model_dir: ", model_dir)
        
        chain = "A"
        task_type = "saprot_regression"
...
if __name__ == "__main__":
    caller = SaProtInferenceRegressionCaller()
