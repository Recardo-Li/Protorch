...
@register_tool
class HHAlignHMM(BaseTool):
    def __init__(self, out_dir: str = f"{ROOT_DIR}/outputs/hhalign_hmm", **kwargs):
        super().__init__(
    ...
    def __call__(self, query_hmm: str, template_hmm: str, mact: float = 0.1, output_path: str = None) -> dict:
        """
        ...
        query_path = os.path.join(self.out_dir, query_hmm) if not os.path.isabs(query_hmm) else query_hmm
        template_path = os.path.join(self.out_dir, template_hmm) if not os.path.isabs(template_hmm) else template_hmm

        if not os.path.exists(query_path) or not os.path.exists(template_path):
            return {"error": f"Input file not found. Checked paths: {query_hmm}, {template_hmm}"}
        ...
        if output_path:
            ...
            save_dir = f"{self.out_dir}/hhalign_hmm/{timestamp}"
...
if __name__ == '__main__':
    ...
    # Test
    hhalign = HHAlignHMM(BASE_DIR)
    
    input_args = {
        "query_hmm": f"example/example.hmm",
        "template_hmm": f"example/protein_sequence.hmm"
        }
