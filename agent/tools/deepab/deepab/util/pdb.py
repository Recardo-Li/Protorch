...
def pdb2fasta(pdb_file, num_chains=None):
    ...
    parser = PDBParser() if pdb_file.endswith('.pdb') else FastMMCIFParser()
...
def get_atom_coords(pdb_file, fasta_file=None):
    p = PDBParser() if pdb_file.endswith('.pdb') else FastMMCIFParser()
...
def cdr_indices(chothia_pdb_file, cdr):
    ...
    parser = PDBParser() if chothia_pdb_file.endswith('.pdb') else FastMMCIFParser()
...
def write_pdb_bfactor(in_pdb_file, out_pdb_file, bfactor):
    parser = PDBParser() if in_pdb_file.endswith('.pdb') else FastMMCIFParser()
