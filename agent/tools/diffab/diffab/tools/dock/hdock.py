...
from agent.tools.diffab.diffab.tools.renumber import renumber as renumber_chothia
...
class HDock(DockingEngine):
    ...
    def __init__(
        ...
        if tmpdir is None:
            self._tmpdir = tempfile.TemporaryDirectory(dir='./tmp/')
    ...
    def set_receptor(self, pdb_path):
        shutil.copyfile(pdb_path, os.path.join(self.tmpdir, 'receptor.pdb'))
        self._has_receptor = True

    def set_ligand(self, pdb_path):
        shutil.copyfile(pdb_path, os.path.join(self.tmpdir, 'ligand.pdb'))
        self._has_ligand = True

    def _dump_complex_pdb(self):
        parser = PDB.PDBParser(QUIET=True)
        model_receptor = parser.get_structure(None, os.path.join(self.tmpdir, 'receptor.pdb'))[0]
        docked_pdb_path = os.path.join(self.tmpdir, 'ligand_docked.pdb')
        ...
        for i, model_ligdocked in enumerate(structure_ligdocked):
            ...
            save_path = os.path.join(self.tmpdir, f"complex_{i}.pdb")
...
class HDockAntibody(HDock):
    ...
    def set_antibody(self, pdb_path):
        heavy_chains, _ = renumber_chothia(pdb_path, os.path.join(self.tmpdir, 'ligand.pdb'))
        self._has_ligand = True
        self._heavy_chain_id = heavy_chains[0]

    def _prepare_lsite(self):
        lsite_content = f"95-102:{self._heavy_chain_id}\n"  # Chothia CDR H3
        with open(os.path.join(self.tmpdir, 'lsite.txt'), 'w') as f:
    ...
    def _prepare_rsite(self):
        ...
        with open(os.path.join(self.tmpdir, 'rsite.txt'), 'w') as f:
    ...
    def dock(self):
        ...
        cmd_pl = [self.createpl_bin, os.path.join(self.tmpdir, "Hdock.out"), os.path.join(self.tmpdir, "ligand_docked.pdb"), "-lsite", os.path.join(self.tmpdir, "lsite.txt")]
        if self._epitope_sites is not None:
            self._prepare_rsite()
            cmd_pl += ["-rsite", os.path.join(self.tmpdir, "rsite.txt")]
        subprocess.run(
            cmd_pl, 
            cwd=self.tmpdir, check=True
        )
