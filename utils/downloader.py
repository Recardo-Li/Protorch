...
class Downloader(MultipleProcessRunner):
    ...
    def __init__(
        self, base_url, save_path, overwrite=False, skip_error_info=False, **kwargs
    ):
        """
        ...
        if not overwrite:
            ...
            self.data = [
                uniprot
                for uniprot in tqdm(self.data, desc="Filtering out existing files...")
    ...
    def _target(self, process_id, data, sub_path, *args):
        for i, uniprot in enumerate(data):
            ...
            self.terminal_progress_bar(
                ...
                i + 1,
                len(data),
                f"Process{process_id} Downloading files...",
            )
...
class AlphaDBDownloader(Downloader):
    ...
    def __init__(self, uniprot_ids, type: str, save_dir: str, **kwargs):
        ...
        super().__init__(
            data=uniprot_ids, base_url=base_url, save_path=save_path, **kwargs
        )
